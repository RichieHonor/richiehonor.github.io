...
#### Capturing Images
This program allowed me to operate two webcams simultaneously, and same the files with a name corresponding to the imput of a barcoder. 
...
#### Machine Learning
After training the computer using plantcv2, this code distinguished plant from background in a photo and stored the number of pixels in the photo in a file that could be used in later analyses.
```{python,eval=F,python.reticulate=F}
#!/usr/bin/env python
# coding: utf-8
...
#### Error Checking
This script allowed me to open and sort through the photos to determine if there were errors on the photos, as well as classify the error type. 
```{python,eval=F,python.reticulate=F}
#!/usr/bin/env python
...
#### Error fixing
This script uses the output of the previous script to rerun the analysis (which can be modified) over photos that were incorrectly identified. 
```{python,eval=F,python.reticulate=F}
#!/usr/bin/env python
